  ...
  try {
    ...
    for (const [collection, documents] of Object.entries(sampleKnowledge)) {
      ...
      for (const doc of documents) {
        ...
        const chunkData = documentChunks.map(chunk => ({
          collection,
          docId: doc.docId,
          title: doc.title,
          chunk: chunk.text,
      ...
      if (chunks.length > 0) {
        const chunkIds = await retrievalService.addChunks(chunks);
...
if (require.main === module) {
  seedKnowledgeBase()
    .then(() => {
      logger.info('Knowledge base seed completed successfully');
      process.exit(0);
    })
    .catch((error) => {
      logger.error({ error }, 'Knowledge base seed failed');
      process.exit(1);
    });
}
